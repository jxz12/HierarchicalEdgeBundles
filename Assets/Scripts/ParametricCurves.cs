...
public class CubicParametricCurve
{
    ...
    public static IEnumerable<Vector3> CubicGradient(Vector3 P1, Vector3 P2, Vector3 P2Gradient, Vector3 P3, int numSegments)
    {
        ...
        for (int i = 0; i <= numSegments; i++)
        {
            ...
            yield return new Vector3(TMG(t, M_c, G_x), TMG(t, M_c, G_y), TMG(t, M_c, G_z));
        }
    }

        
    public static IEnumerable<Vector3> Bezier(Vector3 P1, Vector3 P2, Vector3 P3, Vector3 P4, int numSegments)
    {
        ...
        {
            float t = (float)i / numSegments;
            yield return new Vector3(TMG(t, M_b, G_x), TMG(t, M_b, G_y), TMG(t, M_b, G_z));
        }
    }

    public static IEnumerable<Vector3> BSpline(Vector3 P1, Vector3 P2, Vector3 P3, Vector3 P4, int numSegments, bool includeLastPoint=true)
    {
        ...
        for (int i = 0; i <= n; i++) {
            ...
            yield return new Vector3(TMGUnrolled(t, M_bspline, G_x),
                                     TMGUnrolled(t, M_bspline, G_y),
                                     TMGUnrolled(t, M_bspline, G_z));
        }
    }

    public static IEnumerable<Vector3> PiecewiseBspline(IReadOnlyList<Vector3> P, int segmentsPerSpline)
    {
        if (P.Count < 3) {
            throw new System.Exception("not enough points to make a curved spline");
        }
        var spline = BSpline(P[0], P[0], P[0], P[1], segmentsPerSpline, false);
        spline = spline.Concat(BSpline(P[0], P[0], P[1], P[2], segmentsPerSpline, false));

        for (int i = 0; i < P.Count - 3; i++) {
            spline = spline.Concat(BSpline(P[i], P[i+1], P[i+2], P[i+3], segmentsPerSpline, false));
        }
        int n = P.Count - 1;
        spline = spline.Concat(BSpline(P[n-2], P[n-1], P[n], P[n], segmentsPerSpline, false));
        return spline.Concat(BSpline(P[n-1], P[n], P[n], P[n], segmentsPerSpline, true));
    }
